...
defmodule Guaxinim do
  ...
  def prepare_dst(config) do
    File.mkdir_p!(config.dst_root)
    dst_static = Path.join(config.dst_root, "_static")
  ...
  def run() do
    config = %Config{
      src_root: "c:/Users/tmbb9/Projects/elixir/guaxinim/lib",
      dst_root: "c:/Users/tmbb9/Projects/elixir/guaxinim/literate",
  ...
  def create_index_file(directory, config) do
    abs_directory = Path.join(config.src_root, directory)
    ...
    output_file = Path.join(config.dst_root, file)
    write_with_mkdir(output_file, html)
  end

  def process_file(file, config) do
    abs_file = Path.join(config.src_root, file)
    src_dirname = Path.dirname(abs_file)

    {sibling_directories, sibling_files} =
      child_directories_and_files(src_dirname)
    ...
    html = render_code_page(assigns)
    output_file = Path.join(config.dst_root, file <> ".html")
    write_with_mkdir(output_file, html)
  end
end
